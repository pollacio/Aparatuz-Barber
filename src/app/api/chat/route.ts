...
chat/r
    ...
    Criação da reserva:
    - Após o usuário confirmar explicitamente a escolha (ex: "confirmo", "pode agendar", "quero esse horário"), use a tool createBooking para criar o agendamento e iniciar o pagamento.
    - Parâmetros necessários:
      * serviceId: ID do serviço escolhido
      * date: Data e horário no formato ISO (YYYY-MM-DDTHH:mm:ss) - exemplo: "2025-11-05T10:00:00"
    - Se a criação for bem-sucedida (success: true), informe ao usuário que a reserva foi confirmada com sucesso e informe que ele sera redirecionado para realização do pagamento
    ...
    tools: {
      ...
      createBooking: tool({
        ...
        execute: async ({ serviceId, date }) => {
          console.log("createBookingCheckoutSession", serviceId, date);
          try {
