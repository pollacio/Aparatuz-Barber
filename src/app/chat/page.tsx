...
const ChatPage = () => {
  const { messages, sendMessage, status } = useChat({
    ...
    onFinish: async (data) => {
      ...
      const message = data.message.parts
      const createBookMessage = message.filter((m) => m.type == "tool-createBooking")[0]

      console.log('createBookMessage >>> ', createBookMessage)

      if(createBookMessage){
        const stripe = await stripePromise;
        if (!stripe) return;

        await stripe.redirectToCheckout({
          // @ts-ignore
          sessionId: createBookMessage.output.sessionId,
        });
      }
    },
  });
